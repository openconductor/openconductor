...
export function OnboardingWrapper({ children }: { children: React.ReactNode }) {
  const [firstName, setFirstName] = useState<string>(() => getLocalStorage('firstName', ''));
  const [email, setEmail] = useState<string>(() => getLocalStorage('email', ''));
  ...
  return <OnboardingContext.Provider value={sharedState}>{children}</OnboardingContext.Provider>;
}
